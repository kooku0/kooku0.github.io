        ...
        {data.map((row, i) => (
          <tr key={`outer-row-${i}`}>
            {row.map((innerTableData, j) => (
              <td key={`outer-cell-${i}-${j}`}><InnerTable key={`inner-table-${i}-${j}`} data={innerTableData} /></td>
            ))}
          </tr>
        ))}
      </tbody>
...
const mandalArt = [
  [
    ...
    [
      ...
      ['공부한 내용을 글로 정리하기', '공부한 내용을 잘 설명할 수 있는 능력 기르기', '배우거나 트러블 슈팅한 내용 잘 정리하기']
    ],
    [
      ['주식 매일 한시간 공부하기 & 분석하기', '충동구매 줄이기 (한달 고민하기)', '차트프로 완강하기'],
      ['머신러닝 공부하기', '돈', '데이터 분석 공부하기'],
      ['주식으로 오백만원 이상 수익을 달성하기', '추세 50% 정확도가 넘는 주식예측 모델 만들기', '나의 자산을 정확히 분석하고 있기']
    ],
  ],
  [
    [
      ['배려있게 말하기 & 행동하기', '인사하기', '웃으면서, 눈을 보며 대화하기'],
      ['비난하거나 잘못을 들추지 않기', '동료', '이해하고 존중하기'],
    ...
    [
      ['한달에 와인 두 병이상 마시기', '와인 전문가 자격증 Level3 최고등급 따기', '향을 정확히 분석할 수 있는 능력 기르기'],
