...
WebP는 JPEG, PNG 및 GIF 이미지를 완전히 대체할 수 있으며, lossless and lossy compression 을 모두 지원합니다. `lossless compression` 은 데이터의 손실이 발생하지않 `lossy compression`는 파일크기가 줄어들고 이미지 화질을 떨어지게 하는 압축입니다.
