...
* **Cumulative Layout Shift (CLS)**: 예기치 않은 *layout sift* 에 대한 모든 개별 레이아웃 이동 점수들의 합계를 측정합니다. *layout sift*는 element가 렌더링 된 프레임에서 다음 프레임으로 위치가 변경될때 마다 발생합니다.
...
로깅된 결과를 그래프로 볼 수 있는 서비스가 존재하는데, 바로 [Web Vitals Report](https://github.com/GoogleChromeLabs/web-vitals-report)입니다. GoogleChromeLabs에서 만든 오픈소스로 [Web Service](https://web-vitals-report.web.app/) 도 제공하고 있습니다.
하지만 안타깝게도 GA4를 지원하지 않기에 제가 적용한 Web-vitals는 확인할 수 없었습니다.ㅠㅠ
